...
for i in range(len(dataset)):
	temp_r = R.from_quat([dataset[i][1],dataset[i][2],dataset[i][3],dataset[i][4]])
	temp_m = temp_r.as_matrix()
	a = temp_m.flatten()
	data_all.append([])
	data_all[i].append(dataset[i][0])
	for k in range(len(a)):
		data_all[i].append(a[k])

dataset = []
for i in range(len(data_all)):
	dataset.append([])
	dataset[i].append(data_all[i][3])

dataset = numpy.array(dataset,dtype=numpy.float32)
